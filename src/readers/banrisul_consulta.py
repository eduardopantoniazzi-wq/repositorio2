...
def ler_consulta_banrisul(caminho) -> pd.DataFrame:
    ...
    aguardando: dict | None = None   # segura dados do NSU enquanto espera complemento
    ...
    for linha in linhas:
        ...
        # ── Linha com NSU (e opcionalmente a data no início) ──────────────
        m_nsu = _RE_NSU.search(l)
        if m_nsu:
            if m_nsu.group(1):          # data veio junto com o NSU
        ...
        # ── Linha de complemento (beneficiário) ───────────────────────────
        if aguardando and aguardando.get("data"):
            # Formato: "NOME DO BENEFICIÁRIO - CODIGOBARRAS..."
            # Para Transferências: "AGENCIA - CONTA - DESCRICAO - NOME"
            partes = l.split(" - ")
            operacao = aguardando["operacao"].lower()

            if "transfer" in operacao:
                # "0165 - 3522918906 - DIF SALARIO - ALINE PRISCILA..."
                # Nome fica na última parte
                beneficiario = partes[-1].strip() if len(partes) >= 2 else partes[0]
            elif "arrecad" in operacao:
                # "CORSAN 1 - codigo..." — nome é a primeira parte
                beneficiario = partes[0].strip()
            else:
                # Título: "NOME EMPRESA - codigobarras"
                beneficiario = partes[0].strip()
